...
const tinyKey = "l30kS3uJ80/me1yJmQu63AXxqLlobADjxgx5fE/x0pQ="; // 256-bit key for AES-256
const tinyIV = "Q5XudELyhpRdXf7bHPTC+Q=="; // 128-bit IV
...
export function encryptToHex(text: string) {
	const { iv, encryptedData } = encrypt(text);
	return Buffer.from(`${iv}:${encryptedData}`).toString("hex");
}

export function decryptFromHex(text: string) {
	const decoded = Buffer.from(text, "hex").toString("utf-8");
	const [iv, encryptedData] = decoded.split(":");
	return decrypt({ iv, encryptedData });
}

export function tinyEncrypt(text: string) {
	const ivBuffer = Buffer.from(tinyIV, "base64");
	const keyBuffer = Buffer.from(tinyKey, "base64");
...
export function tinyDecrypt(encrypted: string) {
	const ivBuffer = Buffer.from(tinyIV, "base64");
	const keyBuffer = Buffer.from(tinyKey, "base64");
